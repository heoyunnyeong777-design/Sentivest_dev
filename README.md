...
# .env 파일 생성 (직접 작성)
# 필요한 키를 작성하세요: DATABASE_URL, DEBUG, HOST, PORT, SECRET_KEY, ALLOWED_ORIGINS 등
```
...
### ✅ 완료된 작업
- 프로젝트 구조 생성
- 기본 설정 파일 및 환경 변수 파일 생성 (.gitignore, requirements.txt, .env)
...
- [x] `src/sentivest/config/` 설정 모듈 작성
  - [x] 환경 변수 로드
  - [x] 데이터베이스 설정
  - [x] API 설정
